...
export async function deleteAdminService(id: string) {
  return fetchAdminJson<{ success?: boolean }>(`/services/${id}`, { method: "DELETE" });
}

export async function publishAdminService(id: string) {
  const res = await fetchAdminJson<unknown>(`/services/${id}/publish`, { method: "PATCH" });
  return unwrapData<Service>(res);
}

export async function unpublishAdminService(id: string) {
  const res = await fetchAdminJson<unknown>(`/services/${id}/unpublish`, { method: "PATCH" });
  return unwrapData<Service>(res);
}
...
export async function fetchAdminServiceCategories(params: {
  ...
  if (params.limit) query.set("limit", String(params.limit));
  if (params.status && params.status !== "all") query.set("status", params.status);
  const path = query.toString() ? `/service-categories?${query}` : "/service-categories";
  return fetchAdminJson<ServiceCategoryListResponse>(path, { signal: params.signal });
}
...
export async function createAdminServiceCategory(payload: Record<string, unknown>) {
...
export async function updateAdminServiceCategory(id: string, patch: Record<string, unknown>) {
  return fetchAdminJson<ApiResponse<ServiceCategory>>(`/service-categories/${id}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(patch),
  });
...
export async function publishAdminServiceCategory(id: string) {
  const res = await fetchAdminJson<unknown>(`/service-categories/${id}/publish`, { method: "PATCH" });
  return unwrapData<ServiceCategory>(res);
}

export async function deleteAdminServiceCategory(id: string) {
  return fetchAdminJson<{ success?: boolean }>(`/service-categories/${id}`, { method: "DELETE" });
}
...
export async function uploadServiceImage(file: File) {
  ...
  return fetchAdminJson<{ url: string; publicId?: string }>("/upload/single?folder=services", {
    method: "POST",
    body: formData,
  });
}
