...
const BLOG_TAGS: BlogTag[] = ["news", "habitat", "animal", "project", "education", "conservation"];
import {
  archiveAdminBlog,
  createAdminBlog,
  deleteAdminBlog,
  ...
  if (err.payload && typeof err.payload === "object") {
    ...
    if (typeof payload.message === "string") {
      if (Array.isArray(payload.errors)) {
        const details = (payload.errors as Array<Record<string, unknown>>)
...
export default function BlogEditor({
  ...
  useEffect(() => {
    if (suspendDirtyRef.current || !slugViSynced || !titleVi) return;
    setSlugVi(slugifyText(titleVi));
  }, [titleVi, slugViSynced]);

  useEffect(() => {
    if (suspendDirtyRef.current || !slugEnSynced || !titleEn) return;
    setSlugEn(slugifyText(titleEn));
  }, [titleEn, slugEnSynced]);
  ...
  const hydrateFromBlog = useCallback((blog: Blog) => {
    ...
    setTitleVi(blog.title_i18n?.vi || raw.title || "");
    setTitleEn(blog.title_i18n?.en || "");
    setSlugVi(
      typeof blog.slug_i18n === "string"
        ? blog.slug_i18n
        : blog.slug_i18n?.vi || raw.slug || ""
    );
    setSlugEn(
      typeof blog.slug_i18n === "string"
        ? blog.slug_i18n
    ...
    setExcerptVi(blog.excerpt_i18n?.vi || raw.excerpt || "");
    ...
    setSelectedTags((blog.tags || []).filter((t): t is BlogTag => BLOG_TAGS.includes(t as BlogTag)));
    setIsFeatured(!!blog.isFeatured);
    setSortOrder(blog.sortOrder ?? 0);

    setSeoTitleVi(blog.seoTitle_i18n?.vi || raw.seoTitle || raw.metaTitle || "");
    setSeoTitleEn(blog.seoTitle_i18n?.en || "");
    setSeoDescVi(blog.seoDescription_i18n?.vi || raw.seoDescription || raw.metaDescription || "");
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteAdminBlog(activeBlogId);
      toast.success("Blog deleted");
      if (embedded) {
        onCancel?.();
      } else {
        router.push(`${adminBase}/blogs`);
      }
    } catch (error) {
      toast.error(getErrorMessage(error));
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      if (
        ...
        coverUploading ||
        ogUploading ||
        autoSaveFailCountRef.current >= 3
      ) {
        return;
      }
      ...
      try {
        const payload = buildPayload();
        await updateAdminBlog(activeBlogId, payload);
        dirtyRef.current = false;
        autoSaveFailCountRef.current = 0;
        setLastSavedAt(new Date().toISOString());
      } catch (error) {
        autoSaveFailCountRef.current += 1;
        console.error("[BlogEditor] Auto-save failed:", error);
        if (autoSaveFailCountRef.current >= 3) {
          toast.error("Auto-save disabled after repeated failures. Use manual save.");
        } else {
          toast.error(getErrorMessage(error));
        }
      } finally {
                      ...
                      <button
                        ...
                        className={`rounded-full border px-3 py-1 text-xs font-semibold transition-colors ${
                          checked
                            ...
                            : "border-input bg-background text-muted-foreground hover:border-emerald-400 hover:text-emerald-600"
                        }`}
                      >
                        {tag}
                      </button>
