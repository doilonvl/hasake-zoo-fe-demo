...
import { fetchAdminProjects } from "@/lib/api/projects.admin";
...
const STATUS_LABELS: Record<PublishStatus, string> = { draft: "Draft", published: "Published" };
...
export default function AdminProjectsPage() {
  ...
  const rows = useMemo(
    () =>
      items.map((item) => ({
        item,
        title: resolveLocalizedString(item.title_i18n, locale, "Untitled"),
        location: resolveLocalizedString(item.location_i18n, locale, "-"),
  ...
  return (
    <Dialog.Root
      ...
      onOpenChange={(open) => {
        setEditorOpen(open);
        if (!open) { setActiveProjectId(undefined); setRefreshSeed((v) => v + 1); }
      }}
    >
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Showcase completed projects.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" onClick={() => setRefreshSeed((v) => v + 1)} disabled={loading}>
              <RefreshCcw className="mr-2 h-4 w-4" />Refresh
            </Button>
            <Button onClick={() => { setEditorMode("create"); setActiveProjectId(undefined); setEditorOpen(true); }}>
        ...
        <Card className="p-4">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex flex-1 flex-wrap items-center gap-2">
              <Input value={pendingQuery} onChange={(e) => setPendingQuery(e.target.value)} placeholder="Search title, slug..." className="min-w-[220px] flex-1" />
        ...
        <Card className="overflow-hidden">
          ...
          {!loading && !error && items.length === 0 ? <div className="border-t px-4 py-6 text-sm text-muted-foreground">No projects found.</div> : null}
